...
class StarlightPuzzleGame {
    constructor() {
        ...
        this.sessionCoins = 0;
        this.timeRemaining = 30; // 30 saniye süre
        this.gameTimer = null;
        this.puzzleSize = 4; // 4x4 yap-boz (16 parça)
        this.puzzleBoard = []; // Ana tahta - başlangıçta boş
        this.puzzlePieces = []; // Ayrı duran parçalar
        this.placedPieces = []; // Yerleştirilmiş parçalar
        ...
        // Yap-boz fotoğrafları - prenses resmi
        this.puzzleImages = [
            'prenses.png',
            'prenses.png', 
            'prenses.png',
    ...
    createPuzzleBoard() {
        ...
        if (!boardElement || !piecesContainer) return;

        // Ana tahtayı temizle
        boardElement.style.gridTemplateColumns = `repeat(${this.puzzleSize}, 1fr)`;
        boardElement.innerHTML = '';
        piecesContainer.innerHTML = '';

        // Parçalar container'ını ayarla
        piecesContainer.style.display = 'grid';
        piecesContainer.style.gridTemplateColumns = 'repeat(4, 1fr)';
        piecesContainer.style.gap = '5px';
        piecesContainer.style.padding = '10px';

        // Ana tahtada boş slotlar oluştur
        this.puzzleBoard = [];
        for (let i = 0; i < this.puzzleSize * this.puzzleSize; i++) {
            this.puzzleBoard[i] = null; // Başlangıçta tüm slotlar boş
            
            const slotElement = document.createElement('div');
            slotElement.className = 'puzzle-slot';
            slotElement.dataset.slotId = i;
            slotElement.style.border = '2px dashed #555';
            slotElement.style.borderRadius = '8px';
            slotElement.style.minHeight = '70px';
        ...
        this.puzzlePieces = [];
        const currentImage = this.puzzleImages[(this.currentLevel - 1) % this.puzzleImages.length];
        
        for (let i = 0; i < this.puzzleSize * this.puzzleSize; i++) {
            const pieceElement = document.createElement('div');
            pieceElement.className = 'puzzle-piece draggable';
            pieceElement.draggable = true;
            pieceElement.dataset.pieceId = i;
            
            const row = Math.floor(i / this.puzzleSize);
            const col = i % this.puzzleSize;
            
            // Fotoğraf parçası stilini ayarla
            pieceElement.style.width = '70px';
            pieceElement.style.height = '70px';
            pieceElement.style.backgroundImage = `url('${currentImage}')`;
            pieceElement.style.backgroundSize = `${this.puzzleSize * 70}px ${this.puzzleSize * 70}px`;
            pieceElement.style.backgroundPosition = `-${col * 70}px -${row * 70}px`;
            pieceElement.style.border = '2px solid #fff';
            pieceElement.style.borderRadius = '5px';
            pieceElement.style.cursor = 'grab';
            pieceElement.style.position = 'relative';
            
            // Drag event listeners
            pieceElement.addEventListener('dragstart', (e) => this.handleDragStart(e, i));
            pieceElement.addEventListener('dragend', (e) => this.handleDragEnd(e));
            
            // Touch events for mobile
            pieceElement.addEventListener('touchstart', (e) => this.handleTouchStart(e, i));
    ...
    startNewPuzzle() {
        ...
        console.log('Yeni puzzle başlatıldı! 30 saniye süre.');
    }
    ...
    updateGameUI() {
        ...
        if (progressBar) {
            const progress = ((30 - this.timeRemaining) / 30) * 100;
    ...
    showSuccessAnimation(reward) {
        ...
        animation.innerHTML = `
            ...
            <div class="success-text">4x4 Puzzle Tamamlandı!</div>
    ...
    showPuzzleCompleteModal(reward) {
        ...
        if (finalScore) finalScore.textContent = `${30 - this.timeRemaining} saniyede tamamlandı`;
        ...
        const gameOverHtml = `
            <div class="modal" id="game-over-modal">
                <div class="modal-content">
                    <h2>⏰ Süre Doldu!</h2>
                    <p>Puzzle'ı 30 saniyede çözemediniz.</p>
                    
                    <div class="completion-stats">
                        <div class="stat">
                            <span class="stat-label">Süre:</span>
                            <span class="stat-value">30 saniye</span>
                        </div>
                        <div class="stat">
