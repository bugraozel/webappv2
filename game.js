...
class StarlightPuzzleGame {
    ...
    createPuzzleBoard() {
        ...
        boardElement.style.maxWidth = '320px';
    ...
    renderPuzzleBoard() {
        ...
        for (let i = 0; i < 9; i++) {
            ...
            if (this.puzzleBoard[i] === null) {
                ...
                tileElement.style.background = 'rgba(0,0,0,0.4)';
                tileElement.style.border = '2px dashed rgba(255,255,255,0.3)';
                tileElement.style.borderRadius = '8px';
            } else {
                // Fotoğraf parçası
                ...
                // Doğru background position hesaplaması
                const bgPosX = -(col * 100); // Her parça 100px genişlik
                const bgPosY = -(row * 100); // Her parça 100px yükseklik
                
                tileElement.style.backgroundImage = `url('${this.puzzleImage}')`;
                tileElement.style.backgroundSize = '300px 300px'; // 3x3 = 300px
                tileElement.style.backgroundPosition = `${bgPosX}px ${bgPosY}px`;
                tileElement.style.backgroundRepeat = 'no-repeat';
                tileElement.style.border = '3px solid #fff';
                tileElement.style.cursor = 'pointer';
                tileElement.style.borderRadius = '8px';
                tileElement.style.minHeight = '100px';
                tileElement.style.transition = 'all 0.3s ease';
                tileElement.style.boxShadow = '0 2px 10px rgba(0, 0, 0, 0.3)';

                // Hover efekti
                tileElement.addEventListener('mouseenter', () => {
                    if (window.innerWidth > 480) { // Sadece desktop'ta hover
                        tileElement.style.transform = 'scale(1.05)';
                        tileElement.style.boxShadow = '0 4px 15px rgba(0, 0, 0, 0.4)';
                        tileElement.style.borderColor = 'rgba(255, 215, 0, 0.8)';
                    }
                });
                ...
                tileElement.addEventListener('mouseleave', () => {
                    ...
                    tileElement.style.boxShadow = '0 2px 10px rgba(0, 0, 0, 0.3)';
                    tileElement.style.borderColor = '#fff';
                });
